...
// ── Détection MIME supporté (fix format vidéo) ──────────────────────────────
...
async function startRecording() {
  try {
    // RESET COMPLET avant chaque nouvel enregistrement
    recordedChunks = [];
    recordedBlob = null;
    setDownloadReady(false);

    // Annuler tout URL pendante
    if (lastDownloadUrl) {
...
// ── Téléchargement (fix race condition) ──────────────────────────────────────
function downloadRecording() {
  ...
  // ⚠️ PAS de reset ici — c'est startRecording() qui s'en charge
  // On révoque juste l'URL proprement après le téléchargement
  setTimeout(() => {
    if (lastDownloadUrl) {
      URL.revokeObjectURL(lastDownloadUrl);
      lastDownloadUrl = null;
